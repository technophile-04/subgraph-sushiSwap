...
export function getOrCreateERC20Token(
...
export function getOrCreateMarket(
	...
	for (let i = 0; i < inputTokens.length; i++) {
		...
		inputTokenBalances.push(
			new TokenBalance(token.id, addressHex, BigInt.fromI32(0))
		);
	}
	...
	market.inputTokenTotalBalances = inputTokenBalances.map<string>((tb) =>
		tb.toString()
	);
...
export function getOrCreateOpenPosition(
	...
	let pid = accountPosition.id
		.concat('-')
		.concat(accountPosition.positionCounter.toString());
...
function createPostionSnapshot(
	position: Position,
	transaction: Transaction
): PositionSnapshot {
	let newCounter = position.historyCounter.plus(BigInt.fromI32(1));
	let newSnapshot = new PositionSnapshot(
		position.id.concat('-').concat(newCounter.toString())
	);
...
export function investInMarket(
	...
	transaction.inputTokenAmounts = inputTokenAmounts.map<string>((tb) =>
		tb.toString()
	);
	transaction.outputTokenAmount = outputTokenAmount;
	transaction.rewardTokenAmounts = rewardTokenAmounts.map<string>((tb) =>
		tb.toString()
	);
	...
	let position = getOrCreateOpenPosition(
		event,
		account,
		market,
	...
	position.inputTokenBalances = inputTokenBalances.map<string>((tb) =>
		tb.toString()
	);
	position.outputTokenBalance = outputTokenBalance;
	position.rewardTokenBalances = rewardTokenBalances.map<string>((tb) =>
		tb.toString()
	);
