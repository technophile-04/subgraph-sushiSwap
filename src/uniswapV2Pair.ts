...
export function handleBurn(event: Burn): void {
	...
	marketDayData.inputTokenDailyOutflow = inputTokenDailyOutflow.map<string>(
		(tb) => tb.toString()
	);
	marketDayData.dailyBurnTXs = marketDayData.dailyBurnTXs.plus(
		BigInt.fromI32(1)
	);
	marketDayData.save();
}
...
function createOrUpdatePositionOnMint(
	event: ethereum.Event,
	pair: PairEntity,
	mint: MintEntity
): void {
	let isComplete =
		mint.transferEventApplied && mint.syncEventApplied && mint.mintEventApplied;
	if (!isComplete) {
		return;
	}
	...
	inputTokenAmounts.push(
		new TokenBalance(pair.token0, mint.to!, mint.amount0 as BigInt)
	);
	inputTokenAmounts.push(
		new TokenBalance(pair.token1, mint.to!, mint.amount1 as BigInt)
	);

	let outputTokenBalance = accountLiquidity.balance;
	let token0Balance = outputTokenBalance
		.times(pair.reserve0)
		.div(pair.totalSupply);
	let token1Balance = outputTokenBalance
		.times(pair.reserve1)
		.div(pair.totalSupply);
	let inputTokenBalances: TokenBalance[] = [];
	inputTokenBalances.push(
		new TokenBalance(pair.token0, mint.to!, token0Balance)
	);
	inputTokenBalances.push(
		new TokenBalance(pair.token1, mint.to!, token1Balance)
	);
	...
	// Update market
	updateMarket(event, market, marketInputTokenBalances, pair.totalSupply);
...
export function handleTransfer(event: Transfer): void {
	...
	if (fromHex != ADDRESS_ZERO) {
		...
		accountLiquidityFrom.balance = accountLiquidityFrom.balance.minus(
			event.params.value
		);
	...
	if (fromHex != pairAddressHex) {
		...
		accountLiquidityTo.balance = accountLiquidityTo.balance.plus(
			event.params.value
		);
	...
	if (
		...
		fromHex != pairAddressHex &&
		toHex != pairAddressHex
	) {
	}
}
...
export function handleSync(event: Sync): void {
	...
	if (isSyncOnly) {
		...
		updateMarket(event, market, inputTokenBalances, pair.totalSupply);
	}
}
...
function createOrUpdatePositionOnBurn(
	event: ethereum.Event,
	pair: PairEntity,
	burn: BurnEntity
): void {
	let isComplete =
		burn.transferToPairEventApplied &&
		burn.transferToZeroEventApplied &&
		burn.syncEventApplied &&
		burn.burnEventApplied;
	if (!isComplete) {
		return;
	}
	...
	let inputTokenAmounts: TokenBalance[] = [];
	inputTokenAmounts.push(
		new TokenBalance(pair.token0, burn.to!, burn.amount0 as BigInt)
	);
	inputTokenAmounts.push(
		new TokenBalance(pair.token1, burn.to!, burn.amount1 as BigInt)
	);
	...
	let inputTokenBalances: TokenBalance[] = [];
	inputTokenBalances.push(
		new TokenBalance(pair.token0, burn.from!, token0Balance)
	);
	inputTokenBalances.push(
		new TokenBalance(pair.token1, burn.from!, token1Balance)
	);
...
function checkIncompleteBurnFromLastTransaction(
	event: ethereum.Event,
	pair: PairEntity
): void {
...
function transferLPToken(
	...
	let fromToken0Balance = fromOutputTokenBalance
		.times(pair.reserve0)
		.div(pair.totalSupply);
	let fromToken1Balance = fromOutputTokenBalance
		.times(pair.reserve1)
		.div(pair.totalSupply);
	fromInputTokenBalances.push(
		new TokenBalance(pair.token0, fromAccount.id, fromToken0Balance)
	);
	fromInputTokenBalances.push(
		new TokenBalance(pair.token1, fromAccount.id, fromToken1Balance)
	);
	...
	let toToken0Balance = toOutputTokenBalance
		.times(pair.reserve0)
		.div(pair.totalSupply);
	let toToken1Balance = toOutputTokenBalance
		.times(pair.reserve1)
		.div(pair.totalSupply);
	toInputTokenBalances.push(
		new TokenBalance(pair.token0, toAccount.id, toToken0Balance)
	);
	toInputTokenBalances.push(
		new TokenBalance(pair.token1, toAccount.id, toToken1Balance)
	);
...
function getMarketDayData(event: ethereum.Event): MarketDayData {
	...
	if (marketDayData == null) {
		...
		marketDayData.inputTokensDailySwapInVolume = inputTokensDailySwapInVolume.map<
			string
		>((tb) => tb.toString());
		...
		marketDayData.inputTokensDailySwapOutVolume = inputTokensDailySwapOutVolume.map<
			string
		>((tb) => tb.toString());
		...
		marketDayData.inputTokenDailyInflow = inputTokenDailyInflow.map<string>(
			(tb) => tb.toString()
		);
		...
		marketDayData.feesGenerated = feesGenerated.map<string>((tb) =>
			tb.toString()
		);
		...
		marketDayData.inputTokenTotalBalances = inputTokenTotalBalances.map<string>(
			(tb) => tb.toString()
		);
